...
export default function CustomerForm() {
  ...
  const [customer, setCustomer] = useState<Omit<Customer, 'id'> & { unique_hcp_id: string }>({
    unique_hcp_id: '', // Add unique_hcp_id to the initial state
    name: '',
    specialty: '',
    affiliation: '',
    phone: '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (id && id !== "new") {
        ...
        const { error } = await supabase
          .from("customers")
          .insert([{ ...customer }]); // Include unique_hcp_id in the insert
    ...
    <>
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Add the unique_hcp_id field */}
          <div>
            <label className="block text-sm font-medium text-gray-700">Unique HCP ID</label>
            <input
              type="text"
              required
              value={customer.unique_hcp_id}
              onChange={(e) => setCustomer({ ...customer, unique_hcp_id: e.target.value })}
