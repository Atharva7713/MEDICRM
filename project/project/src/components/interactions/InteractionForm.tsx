...
//         <div>
//           <label className="block text-sm font-medium text-gray-700">HCP</label>
//           <select
//             required
...
//           >
//             <option value="">Select a HCP</option>
...
export default function InteractionForm() {
  ...
  const [users, setUsers] = useState<User[]>([]);
  const [interaction, setInteraction] = useState<Omit<Interaction, 'id'>>({
    customer_id: '', // This will now store `unique_hcp_id`
    user_id: '',
    interaction_date: format(new Date(), "yyyy-MM-dd'T'HH:mm"),
    interaction_type: 'In-person',
    discussion_topics: '',
  ...
  async function loadCustomers() {
    try {
      const { data, error } = await supabase
        .from('customers')
        .select('unique_hcp_id, name') // Select `unique_hcp_id` instead of `id`
        .order('name');
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            >
              <option value="">Select a HCP</option>
              {customers.map((customer) => (
                <option key={customer.unique_hcp_id} value={customer.unique_hcp_id}>
                  {customer.name}
                </option>
              ))}
            </select>
